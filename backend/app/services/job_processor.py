...
def process_job(job_id: int):
    ...
    with Session(db_engine) as session:
        ...
        try:
            ...
            # Refresh cached project stats after successful image creation
            if saved_media and job.project_id:
                try:
                    refresh_project_stats(session, job.project_id)
                    session.commit()
                except Exception as e:
                    print(f"[JobProcessor] Failed to refresh project stats for project {job.project_id}: {e}")
                    session.rollback()
